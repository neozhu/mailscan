...
public class DepartmentAdvancedSpecification : Specification<Department>
{
    public DepartmentAdvancedSpecification(DepartmentAdvancedFilter filter)
    {
       ...
       Query.Where(q => q.Name != null)
             ...
             .Where(q => q.CreatedBy == filter.CurrentUser.UserId, filter.ListView == DepartmentListView.My && filter.CurrentUser is not null)
             .Where(q => q.Created >= start && q.Created <= end, filter.ListView == DepartmentListView.CreatedToday)
             .Where(q => q.Created >= last30day, filter.ListView == DepartmentListView.Created30Days);
